...
// Sundeep code changes start
  ...
  fuel = new Fuel(((Math.round(random(fuelRandomNum))) * 100)/2, 0, canW/10, canH/10, 10, fuelImg);
...
if(isTouching(player, truck4)){
  ...
  fuelDelay = random(canH * 0.18, canH * 0.42) //fix hardcode
  ...
  if (metersSaved !== undefined) {
    ...
    msText = text('High Score', (canW)/2, (canH / 1.25))
